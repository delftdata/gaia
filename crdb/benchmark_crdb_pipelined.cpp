...
const int TARGET_CONCURRENCY = 100;              // Target number of in-flight txns per worker
...
struct PreGenTxn {
    ...
    PreGenTxn(std::string s, bool mh, bool mp, std::string ts) 
        : sql(s), is_multi_home(mh), is_multi_partition(mp), txn_string(ts) {}
};
...
void pre_generate_workload(BenchParams& p) {
    ...
    for (int i = 0; i < TXNS_TO_PREGEN; ++i) {
        ...
        global_txn_pool.emplace_back(sql, mh, mp, txn_id_string);
...
void pipelined_worker(int id, std::string conn_str, BenchParams params, std::atomic<bool>& running, Stats& stats) {
    try {
        std::string local_log_buffer;
        local_log_buffer.reserve(1000000);
        ...
        while (running) {
            pqxx::work txn{c};
            std::queue<const PreGenTxn*> in_flight_metadata;
            ...
            // Scope for the pipeline
            {
                ...
                // 1. FILL
                for (int i = 0; i < params.num_threads && running; ++i) {
                    const auto& txn_data = global_txn_pool[pool_idx % global_txn_pool.size()];
                    pipe.insert(txn_data.sql);
                    in_flight_metadata.push(&txn_data);
                    pool_idx += GENERATOR_THREADS;
                }

                // 2. RETRIEVE
                while (!in_flight_metadata.empty()) {
                    try {
                        ...
                        const PreGenTxn* meta = in_flight_metadata.front();
                        in_flight_metadata.pop();

                        stats.committed++;
                        if (meta->is_multi_home) stats.multi_home++;
                        else if (meta->is_multi_partition) stats.multi_partition++;
                        else stats.single_partition++;

                        stats.latencies.push_back((received_at - sent_at) / 1000000.0);

                        int part_code = meta->is_multi_home ? 1 : (meta->is_multi_partition ? 2 : 0);
                        local_log_buffer += std::to_string(sent_at + id) + "," + std::to_string(id) + ",0," + 
                                            std::to_string(part_code) + ",0,0,0," + std::to_string(sent_at) + "," + 
                                            std::to_string(received_at) + "," + meta->txn_string + "\n";
                    } catch (const std::exception& e) {
                        if (!in_flight_metadata.empty()) in_flight_metadata.pop();
                        stats.aborted++;
                    }
                }
                // Tell the pipeline we are done with this batch
                pipe.complete();
            } // Pipeline is destroyed here
        ...
        LOG(ERROR) << "Thread " << id << " died: " << e.what();
    }
}
...
void worker(int id, std::string conn_str, BenchParams params, std::atomic<bool>& running, Stats& stats) {
    try {
        std::string local_log_buffer; 
        local_log_buffer.reserve(10000); // Pre-allocate memory
        
        pqxx::connection c{conn_str};
        c.prepare("upsert_user", "UPSERT INTO usertable (ycsb_key, field0) VALUES ($1, $2)");
        std::mt19937 gen(id + std::random_device{}());
        std::uniform_real_distribution<double> coin(0.0, 1.0);
        ...
        // Home region boundaries
        long long home_region_start = (long long) params.client_region * keys_per_region;

        // Remote region selection
        std::vector<int> other_regions;
        for (int r = 0; r < params.num_regions; ++r) {
            if (r != params.client_region) other_regions.push_back(r);
        }
        std::uniform_int_distribution<int> remote_region_picker(0, other_regions.size() - 1);
        std::uniform_int_distribution<int> local_partition_picker(0, params.num_partitions - 1);
        ...
        while (running) {
            ...
            bool success = false;
            // Stats trackers for this specific transaction
            bool is_multi_home = coin(gen) < params.multi_home_prob;
            bool is_multi_partition = coin(gen) < params.multi_part_prob;
            std::string txn_string = "SET";
            ...
            try {
                ...
                //LOG(INFO) << "Before region & partition selection";

                // --- 0. REGION & PARTITION SELECTION ---
                if (is_multi_home) {
                    // Pick a remote region different from client region
                    r_reg = other_regions[remote_region_picker(gen)];
                    r_start = (long long) r_reg * keys_per_region;
                } else {
                    // Single home, all keys from client region
                    if (!is_multi_partition) {
                        // Single-partition: all keys from one partition in client region
                        partition = local_partition_picker(gen);
                        partition_start = home_region_start + partition * keys_per_partition;
                        // All keys will be selected from this partition
                    }
                }

                // --- 1. KEY GENERATION (2 Hot, 8 Cold) ---
                
                // A. HOT KEYS (1 Home, 1 Remote/Home)
                // Remote Hot (if MH) or Second Home Hot
                if (is_multi_home) {
                    // Add 1st (remote) hot key
                    keys.push_back(r_start + (params.zipf_skew == 0 ? 
                        std::uniform_int_distribution<long long>(0, hot_per_region - 1)(gen) : 
                        get_zipf_key(params.zipf_skew, hot_per_region)));
                    // Add 2nd (home) hot key
                    keys.push_back(home_region_start + (params.zipf_skew == 0 ? 
                        std::uniform_int_distribution<long long>(0, hot_per_region - 1)(gen) : 
                        get_zipf_key(params.zipf_skew, hot_per_region)));
                } else {
                    if (!is_multi_partition) {
                        // Home Hot
                        keys.push_back(partition_start + (params.zipf_skew == 0 ? 
                            std::uniform_int_distribution<long long>(0, hot_per_partition - 1)(gen) : 
                            get_zipf_key(params.zipf_skew, hot_per_partition)));
                        // Second Home Hot
                        keys.push_back(partition_start + (params.zipf_skew == 0 ? 
                            std::uniform_int_distribution<long long>(0, hot_per_partition - 1)(gen) : 
                            get_zipf_key(params.zipf_skew, hot_per_partition)));                    }
                    else {
                        // Freely pick 2 hot keys from home region
                        keys.push_back(home_region_start + (params.zipf_skew == 0 ? 
                            std::uniform_int_distribution<long long>(0, hot_per_region - 1)(gen) : 
                            get_zipf_key(params.zipf_skew, hot_per_region)));
                        keys.push_back(home_region_start + (params.zipf_skew == 0 ? 
                ...
                // B. COLD KEYS (4 Home, 4 Remote/Home)
                if (is_multi_home) {
                    // Add first 4 (remote) cold keys
                    for (int i = 0; i < 4; i++) {
                        keys.push_back(r_start + (params.zipf_skew == 0 ? 
                    ...
                    if (!is_multi_partition) {
                        // 8 Home Cold keys from selected partition
                        for (int i = 0; i < 8; i++) {
                            keys.push_back(partition_start + (params.zipf_skew == 0 ? 
                                std::uniform_int_distribution<long long>(0, keys_per_partition - hot_per_partition - 1)(gen) + hot_per_partition : 
                                get_zipf_key(params.zipf_skew, keys_per_partition - hot_per_partition) + hot_per_partition));
                        }
                    }
                    else {
                        // Freely pick 8 cold keys from home region
                        for (int i = 0; i < 8; i++) {
                            keys.push_back(home_region_start + (params.zipf_skew == 0 ? 
                                std::uniform_int_distribution<long long>(0, keys_per_region - hot_per_region - 1)(gen) + hot_per_region : 
                                get_zipf_key(params.zipf_skew, keys_per_region - hot_per_region) + hot_per_region));
                ...
                // TODO: Check the correctness of this seems everything si labeled as MH
                std::set<int> nodes_hit;
                std::set<int> regions_hit;
                for(auto k : keys) {
                    int node_id = k / ROWS_PER_PARTITION; // 0, 1, 2, 3
                    int region_id = node_id / 2;          // 0 = West, 1 = East
                    nodes_hit.insert(node_id);
                    regions_hit.insert(region_id);
                    txn_string += ";" + std::to_string(k);
                }

                if (regions_hit.size() > 1) is_multi_home = true;
                if (nodes_hit.size() > 1) is_multi_partition = true;
                txn_string += ";ZpHOk_RANDOM_DATA_" + std::to_string(keys[0] % 999);

                // --- 3. EXECUTION ---

                for (auto k : keys) {
                    // We don't need to send the full SQL string anymore, just the name and data
                    std::string val = "thread_" + std::to_string(id) + "_k" + std::to_string(k % 1000);
                    txn.exec_prepared("upsert_user", k, val);
                }
            ...
            if (success) {
                ...
                // Increment the general committed counter FIRST
                stats.committed++;

                // Prioritize Multi-Home for stats (as it implies multi-partition)
                if (is_multi_home) {
                    stats.multi_home++;
                    stats.multi_partition++; // MH implies MP
                }
                else {
                    if (is_multi_partition) {
                ...
                double latency = (received_at - sent_at) / 1000000.0;
                stats.latencies.push_back(latency);

                // Sample to transactions.csv
                if (coin(gen) < params.sample_rate) {
                    int part_code = is_multi_home ? 1 : (is_multi_partition ? 2 : 0);
                    // Write to local string, NO LOCKING
                    local_log_buffer += std::to_string(sent_at + id) + "," + std::to_string(id) + ",0," + 
                                        std::to_string(part_code) + ",0,0,0," + std::to_string(sent_at) + "," + 
                                        std::to_string(received_at) + "," + txn_string + "\n";
                }
            }
        }
        std::lock_guard<std::mutex> lock(log_mutex);
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 2) {
        LOG(INFO) << "Usage: ./benchmark_crdb <IP> <threads> <read_%> <skew> <multi_part_%> <multi_home_%> <num_partitions> <num_regions> <seconds> <client_region> <workload_name>";
        return 1;
    }
    ...
    std::vector<Stats> all_stats(GENERATOR_THREADS);
    ...
    // 4. Spawn Pipelined Workers
    for (int i = 0; i < GENERATOR_THREADS; ++i) {
        threads.emplace_back(pipelined_worker, i, conn_str, p, std::ref(running), std::ref(all_stats[i]));
    }
    
    // Old version with simple worker (not pipelined). Is much slower.
