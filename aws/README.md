...
5. Test DB service status `python3 tools/admin.py status --image omraz/seq_eval:latest aws/conf_files/tpcc/aws_tpcc_calvin.conf -u ubuntu`
...
7. Run a whole scenario `python3 tools/run_config_on_remote.py -i omraz/seq_eval:latest -m ubuntu@54.177.253.33 -s server_skew -w tpcc -c aws/conf_files/tpcc/aws_tpcc_calvin.conf -u ubuntu -bl True -db calvin 2>&1 | tee scenario_$(date +"%d-%m-%y_%H-%M-%S").log`
